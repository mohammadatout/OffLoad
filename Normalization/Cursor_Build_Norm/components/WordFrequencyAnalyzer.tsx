...
import { Filter, ChevronDown, ChevronRight, Plus } from 'lucide-react';
import { WordFrequency } from '@/lib/types';
import { calculateWordFrequency } from '@/lib/dataProcessing';
              ...
              <div className="flex items-center gap-2">
                <div ref={columnMenuRef} className="relative">
                  <button
                    ...
                    className={cn(
                      'h-8 min-w-[180px] max-w-[220px] flex items-center justify-between gap-2 rounded-lg border border-light-border dark:border-dark-border bg-white dark:bg-dark-bg px-2.5 text-xs text-left text-gray-900 dark:text-gray-100',
                  ...
                  >
                    ...
                    <ChevronDown
                      className={cn(
                        'w-3.5 h-3.5 shrink-0 text-gray-500 transition-transform',
                  ...
                  {columnMenuOpen && (
                    <ul
                      ...
                      className="absolute right-0 z-30 mt-1 w-[260px] max-h-48 overflow-y-auto rounded-lg border border-light-border dark:border-dark-border bg-white dark:bg-dark-bg py-1 shadow-md"
                    >
                      {columns.map((column) => {
                ...
                <div className="flex items-center gap-1">
                  <span className="text-[11px] text-gray-500 dark:text-gray-400">Top</span>
                  <select
                    value={showTopN}
                    onChange={(e) => setShowTopN(Number(e.target.value))}
                    disabled={frequencies.length === 0}
                    className="h-8 px-2.5 rounded-lg border border-light-border dark:border-dark-border bg-white dark:bg-dark-bg text-xs disabled:opacity-50"
                  >
                    <option value={25}>25</option>
                    <option value={50}>50</option>
                    <option value={100}>100</option>
                    <option value={frequencies.length || 0}>All</option>
                  </select>
                </div>
              </div>
            </div>
